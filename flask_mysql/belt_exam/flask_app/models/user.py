...
class User:
    ...
    def validate_register_form(data):
        ...
        if not data["first_name"] and not data["last_name"] and not data["email"] and not data['password'] and not data['confirm_password']:
            is_valid = False 
            flash("All fields required",'error20')
        else:
            ...
            if email:
                is_valid = False
                flash("Account must not already exists",'error5')
            if not data["email"]:
                is_valid = False
                flash("You must put a email",'error6')
            elif not EMAIL_REGEX.match(data['email']):
                is_valid = False
                flash("Email must be a valid Email",'error7')

            if len(data['password']) < 8:
                is_valid = False
                flash("Passwords minimum 8 characters",'error8')
            elif data['password'] != data['confirm_password']:
                is_valid = False
                flash("PW and confirm PW must match.",'error9')
        return is_valid
    @staticmethod
    def validate_login(data):
        ...
        if not data["email_login"] and not data["password_login"]:
            is_valid = False
            flash("User must be logged in to see any of the site's internal pages",'error10')
        else:
